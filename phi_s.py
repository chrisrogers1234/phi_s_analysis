...
def load_data_from_listing(file_listing):
    data_list = []
    for filename in [file_listing["rf"], file_listing["ac"]]:
    ...
    pair = {"RF":data_list[0], "signal":data_list[1]}
...
def plot(plot_dir, data_list_pairs):
    ...
    for i, data in enumerate(data_list_pairs):
        ...
        if str(round(voltage, 2)) in rf_list:
            print "Using", voltage
    ...
    for i, data in enumerate(data_list_pairs):
        ...
        print "  Plotting data"
        y_range = common.min_max(data["RF"]["voltage_list"]+data["signal"]["voltage_list"])
...
def main(args):
    ...
    if '--process' in args:
        ...
        data_list_pairs = [load_data_from_listing(i) for i in file_index[16:]]
        ...
        common.make_root_canvas("FINISHED")
